...
def create_drink(jwt):
    ...
    try:
        ...
        # On sucessful, return the json object with the new drink.
        # You can view this using Postman on the backend server
        return jsonify({
            'drinks': drink.long(),
            'success': True,
        }), 200
...
def patch_drink(jwt, id):
    ...
    try:
        ...
        # it should respond with a 404 error if <id> is not found
        if drink:
            # Since drink id was found in db,
            # lets update the newly submitted details if any
            if data.get('title'):
            ...
            # Return 404 error because this drink id
            # was not found. example id = 10000
            return json.dumps({
                'success': False,
