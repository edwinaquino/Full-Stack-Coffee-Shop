...
Ionic ships with a useful development server which detects changes and transpiles as you work. The application is then accessible through the browser on a localhost port. To run the development server, cd into the `frontend` directory and run the following command to run a local server on the local IP address using HTTPS protocol.

IMPORTANT: You must run on HTTPS secured protocol

```bash
ionic serve -c --ssl --host=127.0.0.1 -p 3000
```

Open local server with your browser:

https://127.0.0.1:3000/tabs/drink-menu
